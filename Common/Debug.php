...
class Debug
{
    ...
    public static function start($key = 'ALL')
    {
        if (!self::$xhprof && Config::getField('project', 'xhprof', 0) && \function_exists('xhprof_enable')) {
            require(houphp::getLibPath() . DS . 'xhprof_lib' . DS . 'utils' . DS . 'xhprof_lib.php');
            require(houphp::getLibPath() . DS . 'xhprof_lib' . DS . 'utils' . DS . 'xhprof_runs.php');
    ...
    private static function getTrace()
    {
        ...
        for ($i = 2, $n = count($traces); $i < $n && $i < 7; $i++) {
            ...
            if (isset($trace['file'])) {
                $fileInfo = str_replace(houphp::getRootPath() . '/', '', $trace['file']) . ':' . $trace['line'];
            } else {
                $fileInfo = '';
            }
