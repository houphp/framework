...
use Houphp\Common\Debug;
use Houphp\Common\Formater;
use Houphp\Common\Log;
...
class Houphp
{
    ...
    public static function run($rootPath, $run = true, $configPath = null)
    {
        ...
        if ('Ant' == $serverMode) { //ant模式的约定
            ...
            $antLibPath = Houphp::getRootPath() . DS . '..' . DS . 'ant-lib';
            if (is_dir($antLibPath)) {   //compsoer方式，此目录并不存在，不需要加到libpath
                if (is_array(self::$libPath)) {
                    self::$libPath += [
                        'ant-lib' => $antLibPath,
                        'ant-rpc' => Houphp::getRootPath() . DS . '..' . DS . 'ant-rpc',
                    ];
                } else {
                    self::$libPath = [self::$libPath];
                    self::$libPath += [
                        'ant-lib' => Houphp::getRootPath() . DS . '..' . DS . 'ant-lib',
                        'ant-rpc' => Houphp::getRootPath() . DS . '..' . DS . 'ant-rpc',
                    ];
                }
            }
