...
namespace Houphp\Socket\Adapter;

use Houphp\Core\Config;
use Houphp\Socket\IServer;
use Houphp\Socket\Callback;
        ...
        switch ($this->config['server_type']) {
            ...
            case self::TYPE_WEBSOCKETS:
                if (!($client instanceof Callback\SwooleWebSocket)) {
                    throw new \Exception('client must instanceof Houphp\Socket\Callback\SwooleWebSocket');
                }
                break;
            case self::TYPE_HTTP:
            case self::TYPE_HTTPS:
                if (!($client instanceof Callback\SwooleHttp)) {
                    throw new \Exception('client must instanceof Houphp\Socket\Callback\SwooleHttp');
                }
                break;
            case self::TYPE_UDP:
                if (!($client instanceof Callback\SwooleUdp)) {
                    throw new \Exception('client must instanceof Houphp\Socket\Callback\SwooleUdp');
                }
                break;
            default:
                if (!($client instanceof Callback\Swoole)) {
                    throw new \Exception('client must instanceof Houphp\Socket\Callback\Swoole');
                }
                break;
        }
        $this->client = $client;
